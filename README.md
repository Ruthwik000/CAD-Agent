...
An [online version](https://openjscad.xyz/), [self hostable web based ui](./packages/web), as [CLI](./packages/cli) (command-line interface) for server-side computations with Node.js, an [AI Agent](./packages/ai-agent) for generating CAD scripts with natural language, as well as an experimental [desktop app](./packages/desktop) or [individual Node.js packages](./packages/README.md)!!
...
**NEW!** 🤖 Generate JSCAD scripts using natural language with our AI Agent powered by Groq API!

The AI Assistant is now **fully integrated** into the JSCAD web application. Simply describe what you want to create (e.g., "Create a simple car" or "Make a house with a roof"), and the AI will generate the JSCAD code and insert it directly into the editor!
...
3. Enter your Groq API key when prompted (get one free at https://console.groq.com)
4. Start creating with natural language!

For detailed instructions, see the [AI Chat Integration Guide](./packages/web/AI_CHAT_INTEGRATION.md).

**Note:** A standalone AI agent package is also available in `packages/ai-agent/` if you prefer a separate application.
